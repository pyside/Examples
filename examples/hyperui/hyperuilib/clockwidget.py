...
class ClockWidget(QGraphicsWidget):
    def __init__(self, parent):
        ...
        self._fontColor = QColor(Resource.stringValue(
            "clock-widget/font-color"))
        self._dayFont = defaultFont
        self._dayFont.setBold(True)
        self._dayFont.setPixelSize(Resource.intValue(
            "clock-widget/day-font-size"))
        self._labelFont = defaultFont
        self._labelFont.setPixelSize(Resource.intValue(
            "clock-widget/label-font-size"))

        self._weekDayFont = defaultFont
        self._weekDayFont.setPixelSize(Resource.intValue(
            "clock-widget/wday-font-size"))
        ...
        self._eventsPixmapRect = Resource.value(
            "clock-widget/events-pixmap-rect")
        self._eventsInnerOffset = Resource.intValue(
            "clock-widget/events-inner-offset")
        self._eventsInnerDiameter = Resource.intValue(
            "clock-widget/events-inner-diameter")
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawText(self._dayRect, Qt.AlignHCenter | Qt.AlignVCenter,
                         dateTime.toString("dd"))
        painter.setFont(self._weekDayFont)
        painter.drawText(self._weekDayRect, Qt.AlignHCenter | Qt.AlignVCenter,
                         dateTime.toString("ddd").upper())
        painter.drawPixmap(self._eventsPixmapRect.x(),
                           self._eventsPixmapRect.y(), self._eventsPixmap)

        # calculate min/hour pointer angles
        time = dateTime.time()
        hourAngle = 180 + qRound(30.0 * (time.hour() + time.minute() / 60.0))
        minuteAngle = \
            180 + qRound(6.0 * (time.minute() + time.second() / 60.0))

        # paint minute pointer
        painter.save()
        painter.translate(self._middlePoint)
        painter.rotate(minuteAngle)
        painter.drawPixmap(-self._minutePointer.width() / 2, 0,
                           self._minutePointer)
        painter.restore()
        ...
        painter.drawPixmap(-self._hourPointer.width() / 2, 0,
                           self._hourPointer)
        painter.restore()
        ...
        for event in self._events:
            ...
            painter.drawRoundedRect(
                bx + 4, int(by + 0.3 * self._labelHeight), int(0.1 * bw),
                int(0.5 * self._labelHeight), 2, 2)

            font.setBold(True)
            painter.setFont(font)
            drawTextWithShadow(
                painter, int(bx + 0.13 * bw),
                int(by + 0.75 * self._labelHeight),
                event._begin.toString("hh:mm"), Qt.white)

            font.setBold(False)
            painter.setFont(font)
            drawTextWithShadow(
                painter, int(bx + 0.3 * bw),
                int(by + 0.75 * self._labelHeight),
    ...
    def updateEvents(self):
        ...
        for event in self._events:
            ...
            # drawPie parameters must be specified in 1/16th of a degree and
            # it's counter-clockwise. so we must adjust to the right values.
            ia = ia * -16 + 90 * 16
            ib = ib * -16
            ...
            gradient = QRadialGradient(diameter / 2, diameter / 2,
                                       diameter / 2 - 35)
            ...
            gradient2 = QRadialGradient(diameter / 2, diameter / 2,
                                        diameter / 2)
            ...
            gradient3 = QRadialGradient(diameter / 2, diameter / 2,
                                        diameter / 2)
