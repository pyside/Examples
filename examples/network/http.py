...
class HttpWindow(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.statusLabel = QtGui.QLabel(self.tr(
        ...
        self.connect(self.http, QtCore.SIGNAL(
    ...
    def downloadFile(self):
        ...
        if not self.outFile.open(QtCore.QIODevice.WriteOnly):
            QtGui.QMessageBox.information(
                self, self.tr("HTTP"), self.tr(
                    "Unable to save the file %(name)s: %(error)s.")
                % {'name': fileName, 'error': self.outFile.errorString()})
            self.outFile = None
            return
        ...
        self.progressDialog.setLabelText(self.tr("Downloading %s.")
                                         % (fileName))
    ...
    def httpRequestFinished(self, requestId, error):
        ...
        if error:
            ...
            self.statusLabel.setText(self.tr(
                "Downloaded %s to current directory.") % (fileName))
