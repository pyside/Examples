...
class Launcher(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.connect(parentPageAction1, QtCore.SIGNAL("triggered()"), self,
                     QtCore.SIGNAL("showPage()"))
        self.connect(parentPageAction2, QtCore.SIGNAL("triggered()"), self,
                     QtCore.SIGNAL("showPage()"))
        self.connect(parentPageAction3, QtCore.SIGNAL("triggered()"), self,
                     QtCore.SIGNAL("showPage()"))
        self.connect(fullScreenAction, QtCore.SIGNAL("triggered()"),
        ...
        self.connect(self.resizeTimer, QtCore.SIGNAL("timeout()"),
        ...
        self.connect(self, QtCore.SIGNAL("windowResized()"),
                     self.redisplayWindow, QtCore.Qt.QueuedConnection)

        self.setCentralWidget(self.display)
        self.setMaximumSize(
            QtGui.QApplication.desktop().screenGeometry().size())
    ...
    def initHelpEngine(self):
        self.helpRootUrl = QtCore.QString(  # Isn't supported by PySide
        ...
        oldDir = self.helpEngine.customValue(
            "docDir", QtCore.QVariant(QtCore.QString())).toString()
        if oldDir != qtDocRoot:
    ...
    def setup(self):
        ...
        if demoCategories + exampleCategories <= 0:
            QtGui.QMessageBox.warning(
                self, self.tr("No Examples or Demos found"),
        ...
        for category in self.categories:
            self.maximumLabels = \
                max(len(self.categories[category]['examples']) + 1,
                    self.maximumLabels)
        ...
        self.categories['[main]']['description'] = mainDescription + self.tr(
            "<p>Press <b>Escape</b>, <b>Backspace</b>, or <b>%1</b> to return "
            "to a previous menu.</p> <p>Press <b>%2</b> to switch between "
            "normal and full screen modes.</p> <p>Use <b>%3</b> to exit the "
            "launcher.</p>").arg(QtCore.QString(QtGui.QKeySequence(
                self.tr("Alt+Left")))).arg(QtCore.QString(QtGui.QKeySequence(
                    self.tr("Ctrl+F")))).arg(QtCore.QString(QtGui.QKeySequence(
                        self.tr("Ctrl+Q"))))
    ...
    def executeAction(self, action):
        if action == "parent":
            ...
            if len(self.runningExamples) == 0:
                self.connect(self.display, QtCore.SIGNAL("displayEmpty()"),
                             self.close)
                self.display.reset()
            else:
                self.close()
    ...
    def launchExample(self, uniquename):
        ...
        self.connect(process, QtCore.SIGNAL("finished(int)"),
        ...
        if self.examples[uniquename]['changedirectory'] == 'true':
            process.setWorkingDirectory(
                self.examples[uniquename]['absolute path'])
    ...
    def showCategories(self):
        ...
        title = TitleShape(self.tr("PyQt Examples and Demos"),
                           self.titleFont, QtGui.QPen(QtGui.QColor("#a6ce39")),
                           QtCore.QPointF(),
                           QtCore.QSizeF(0.5*self.width(), 4*verticalMargin))

        title.setPosition(QtCore.QPointF(
            self.width() / 2 - title.rect().width() / 2,
            -title.rect().height()))
        ...
        exitButton.setTarget(QtCore.QPointF(2*horizontalMargin,
        ...
        for category in categories:
            ...
            background.setTarget(
                QtCore.QPointF(2*horizontalMargin, background.position().y()))
        ...
        exitBackground = PanelShape(
            exitPath, QtGui.QBrush(QtGui.QColor("#a6ce39")), QtGui.QBrush(
                QtGui.QColor("#c7f745")),
            QtGui.QPen(QtCore.Qt.NoPen), QtCore.QPointF(startPosition),
        ...
        exitBackground.setTarget(
            QtCore.QPointF(2*horizontalMargin, exitBackground.position().y()))
        ...
        qtShape = ImageShape(self.qtLogo,
                             QtCore.QPointF(2*horizontalMargin-extra,
                             -imageHeight), qtMaxSize, 0,
        ...
        trolltechShape = \
            ImageShape(self.rbLogo,
                       QtCore.QPointF(self.width()-2 * horizontalMargin
                       - trolltechMaxSize.width() + extra,
                       -imageHeight), trolltechMaxSize, 0,
    ...
    def showExampleDocumentation(self, uniqueName):
        self.disconnect(self.display, QtCore.SIGNAL("displayEmpty()"),
                        self.resizeWindow)
        self.disconnect(self.display, QtCore.SIGNAL("displayEmpty()"),
                        self.close)
        ...
        if self.assistantProcess.state() != QtCore.QProcess.Running:
            ...
            if not self.assistantProcess.waitForStarted():
                QtGui.QMessageBox.critical(
                    None, "Qt Demo", "Could not start Qt Assistant.")
                return
    ...
    def showExamples(self, category):
        ...
        for example in self.categories[self.currentCategory]['examples']:
            ...
            path.addRect(
                -2*extra, -extra, maxWidth + 4*extra, textHeight+2*extra)
        ...
        buttonBackground = PanelShape(
            ...
            QtGui.QBrush(QtGui.QColor("#c7f745")),
            QtGui.QPen(QtCore.Qt.NoPen), QtCore.QPointF(startPosition),
            QtCore.QSizeF(maxWidth + 10*extra, textHeight + 2*extra))
    ...
    def showExampleSummary(self, uniquename):
        ...
        space = bottomMargin - topMargin
        step = min(title.rect().height() / self.fontRatio,
                   (bottomMargin + 4.8*verticalMargin
                   - topMargin)/self.maximumLabels)
        footerTextHeight = self.fontRatio * step

        leftMargin = 3*horizontalMargin
        rightMargin = self.width() - 3*horizontalMargin

        if self.examples[self.currentExample] in ('description'):
            description = DocumentShape(
            ...
            description.setPosition(
                QtCore.QPointF(description.position().x(),
                               0.8*self.height()-description.rect().height()))

            self.display.appendShape(description)
            space = description.position().y() - topMargin - 2*verticalMargin

        if self.examples[self.currentExample] in ('image files'):
            image = QtGui.QImage(
                self.examples[self.currentExample]['image files'][0])
            imageMaxSize = QtCore.QSizeF(
                self.width() - 8*horizontalMargin, space)

            self.currentFrame = ImageShape(
                image, QtCore.QPointF(self.width()-imageMaxSize.width()/2,
        ...
        backButton = TitleShape(
            self.currentCategory, self.font(),
            QtGui.QPen(QtCore.Qt.white),
            QtCore.QPointF(0.1*self.width(),
                           self.height()), QtCore.QSizeF(maxSize),
        ...
        if self.examples[self.currentExample] in ('absolute path'):
        ...
        if self.examples[self.currentExample]['document path']:
            ...
            if rightMargin == 0.0:
                ...
                documentCaption.setPosition(
                    QtCore.QPointF(leftMargin/2 + rightMargin/2 -
                                   documentCaption.rect().width()/2,
                                   self.height()))
    ...
    def showParentPage(self):
        self.slideshowTimer.stop()
        self.disconnect(self.slideshowTimer, QtCore.SIGNAL("timeout()"),
                        self.updateExampleSummary)
    ...
    def updateExampleSummary(self):
        if self.examples[self.currentExample] in ('image files'):
            self.currentFrame.metadata["fade"] = -15
            self.currentFrame.setTarget(QtCore.QPointF(
                (self.currentFrame.position() -
            ...
            self.currentFrame = ImageShape(
                image, QtCore.QPointF(imagePosition), QtCore.QSizeF(imageSize))
            self.currentFrame.metadata["fade"] = 15
            self.currentFrame.setTarget(QtCore.QPointF(
                self.width()/2-imageSize.width()/2, imagePosition.y()))
    ...
    def closeEvent(self, event):
        if len(self.runningExamples) > 0:
            if QtGui.QMessageBox.warning(
                self, self.tr("Examples Running"), self.tr(
                    "There are examples running. Do you really want to exit?"),
    ...
    def resizeEvent(self, event):
        ...
        self.documentFont.setPointSizeF(
            min(self.documentFont.pointSizeF()*self.width()/640.0,
    ...
    def toggleFullScreen(self):
        ...
        self.connect(self.display, QtCore.SIGNAL("displayEmpty()"),
                     self.resizeWindow, QtCore.Qt.QueuedConnection)
        self.display.reset()
    ...
    def addTitle(self, title, verticalMargin):
        ...
        newTitle = TitleShape(
            title, self.titleFont, QtGui.QPen(QtCore.Qt.white),
            QtCore.QPointF(titlePosition),
            QtCore.QSizeF(0.5*self.width(), 2*verticalMargin),
            QtCore.Qt.AlignHCenter | QtCore.Qt.AlignTop)
        newTitle.setPosition(QtCore.QPointF(-newTitle.rect().width(),
                                            titlePosition.y()))
        newTitle.setTarget(QtCore.QPointF(0.25*self.width(),
                                          titlePosition.y()))
    ...
    def addTitleBackground(self, titleShape):
        ...
        backgroundPath.addRect(0, -titleShape.rect().height()*0.3,
                               self.width(), titleShape.rect().height()*1.6)

        titleBackground = PanelShape(
            backgroundPath, QtGui.QBrush(QtGui.QColor("#a6ce39")),
        ...
        titleBackground.setTarget(QtCore.QPointF(0.0,
                                                 titleShape.position().y()))
    ...
    def readExampleDescription(self, parentNode):
        ...
        while not node.isNull():
            if node.isText():
                ...
                description += beginTag + self.readExampleDescription(node)
                + endTag
    ...
    def readInfo(self, resource, dir_):
        ...
        self.qtLogo.load(self.imagesDir.absoluteFilePath(
            ":/images/qt4-logo.png"))
        self.rbLogo.load(self.imagesDir.absoluteFilePath(
            ":/images/rb-logo.png"))
            ...
            if categoryDir.cd(categoryDirName):
                ...
                for j in range(exampleNodes.length()):
                    ...
                    if not categoryDocName.isEmpty():
                        docName = categoryDocName + "-" + exampleFileName
                        + ".html"
                    else:
                        docName = categoryDirName + "-" + exampleFileName
                        + ".html"
                    ...
                    self.examples[uniqueName]['changedirectory'] = \
                        element.attribute("changedirectory", "true")
    ...
    def addVersionAndCopyright(self, rect):
        versionCaption = TitleShape(
            QtCore.QString("Qt %1").arg(QtCore.QT_VERSION_STR),
        ...
        copyrightCaption = TitleShape(QtCore.QString(
            "Copyright \xa9 2005-2006 Trolltech AS"), self.font(),
            QtGui.QPen(QtGui.QColor(0, 0, 0, 0)), QtCore.QPointF(
                rect.topLeft()), QtCore.QSizeF(
    ...
    def findDescriptionAndImages(self, uniqueName, docName):
        if self.documentationDir.exists(docName):
            ...
            exampleFile = QtCore.QFile(self.documentationDir.absoluteFilePath(
                docName))
...
#            categoryDirName = \
#               re.sub(".*/examples(.*)", r"%s\1"
#               % QtCore.QLibraryInfo.location(
#                   QtCore.QLibraryInfo.ExamplesPath), categoryDirName)
#        else:
#            categoryDirName = \
#               re.sub(".*/demos(.*)", r"%s\1"
#               % QtCore.QLibraryInfo.location(QtCore.QLibraryInfo.DemosPath),
#                                              categoryDirName)
#        categoryDir = QtCore.QDir(categoryDirName)
        if categoryDir.exists("README"):
            ...
            while not inputStream.atEnd():
                ...
                while at != -1:
                    if openQuote:
                        line.replace(at, 1, QtCore.QChar(
                            QtCore.QChar.Punctuation_InitialQuote))
                    else:
                        line.replace(at, 1, QtCore.QChar(
                            QtCore.QChar.Punctuation_FinalQuote))
            ...
            self.categories[categoryName]['description'] = \
