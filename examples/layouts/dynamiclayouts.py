...
class Dialog(QDialog):
    ...
    def rotateWidgets(self):
        ...
        for i in range(count//2):
            self.rotableLayout.addWidget(self.rotableWidgets[count - i - 1], 0,
                                         i)
    ...
    def buttonsOrientationChanged(self, index):
        ...
        orientation = Qt.Orientation(int(
            self.buttonsOrientationComboBox.itemData(index)))
        ...
        if orientation == Qt.Horizontal:
            ...
            self.resize(self.size() + QSize(-oldSizeHint.width(),
                                            newSizeHint.height()))
        else:
            self.mainLayout.addWidget(self.buttonBox, 0, 3, 2, 1)
            self.resize(self.size() + QSize(newSizeHint.width(),
                        -oldSizeHint.height()))
    ...
    def show_help(self):
        QMessageBox.information(
            self, "Dynamic Layouts Help",
            "This example shows how to change layouts dynamically.")
    ...
    def createButtonBox(self):
        ...
        rotateWidgetsButton = self.buttonBox.addButton(
