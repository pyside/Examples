...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        yearEdit.setDateRange(QtCore.QDate(1753, 1, 1), QtCore.QDate(8000, 1, 1))
        ...
        self.connect(monthCombo, QtCore.SIGNAL("activated(int)"), self.setMonth)
        self.connect(yearEdit, QtCore.SIGNAL("dateChanged(QDate)"), self.setYear)
        self.connect(self.fontSizeSpinBox, QtCore.SIGNAL("valueChanged(int)"), self.setfontSize)
    ...
    def insertCalendar(self):
        ...
        date = QtCore.QDate(self.selectedDate.year(), self.selectedDate.month(), 1)
        ...
        while date.month() == self.selectedDate.month():
            ...
            if date == QtCore.QDate.currentDate():
                cellCursor.insertText(QtCore.QString("%1").arg(date.day()), highlightedFormat)
            else:
                cellCursor.insertText(QtCore.QString("%1").arg(date.day()), format)
    ...
    def setMonth(self, month):
        self.selectedDate = QtCore.QDate(self.selectedDate.year(), month + 1, self.selectedDate.day())
        self.insertCalendar()
    
    def setYear(self, date):
        self.selectedDate = QtCore.QDate(date.year(), self.selectedDate.month(), self.selectedDate.day())
        self.insertCalendar()
