...
class MainWindow(QtGui.QMainWindow):
    ...
    def setupFileMenu(self):
        ...
        newFileAct.setShortcut(QtGui.QKeySequence(
            self.tr("Ctrl+N", "File|New")))
        self.connect(newFileAct, QtCore.SIGNAL("triggered()"), self.newFile)
        fileMenu.addAction(newFileAct)

        openFileAct = QtGui.QAction(self.tr("&Open..."), self)
        openFileAct.setShortcut(QtGui.QKeySequence(
            self.tr("Ctrl+O", "File|Open")))
...
class Highlighter(QtCore.QObject):
    ...
    def addToDocument(self, doc):
        self.connect(doc, QtCore.SIGNAL("contentsChange(int, int, int)"),
                     self.highlight)
